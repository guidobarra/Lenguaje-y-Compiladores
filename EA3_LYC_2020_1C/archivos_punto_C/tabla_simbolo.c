...
int tsCrearArchivo() {
	...
	fprintf(archivo, "%-50s%-13s%-50s%-12s\n", "Nombre", "Tipo", "Valor", "Longitud");
	...
	for (i = 0; i < posicion_en_ts; i++) {
		...
		if (tablaSimbolos[i].tipo == T_INTEGER || 
			...
			fprintf(archivo, "%-50s%-13s\n", tablaSimbolos[i].nombre, obtenerNombreTipo(tablaSimbolos[i].tipo));
		} else {
			fprintf(archivo, "%-50s%-13s%-50s%-12s\n", 
													aConstante(tablaSimbolos[i].nombre), 
													obtenerNombreTipo(tablaSimbolos[i].tipo), 
													tablaSimbolos[i].valor, 
													tablaSimbolos[i].longitud);

		}
		
		//printf("%-13s", obtenerNombreTipo(tablaSimbolos[i].tipo));
